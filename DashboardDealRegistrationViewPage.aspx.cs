...
{
    public partial class DashboardDealRegistrationViewPage : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (vSession.User != null)
                {
                    if (vSession.User.CompanyType == Types.Vendors.ToString())
                    {
                        ...
                        {
                            Response.Redirect(ControlLoader.Dashboard(vSession.User, "deals-view"), false);
                            return;
                        }
                        else if (vSession.User.Id == CustomVendorID)
                        {
                            Response.Redirect(ControlLoader.Dashboard(vSession.User, "deals-view-365"), false);
                            return;
                        }
                    }
                    else
        ...
        private void LoadCurrency(string selectedCurID)
        {
            ...
            if (currCountries.Count > 0)
            {
                ...
                foreach (ElioCurrenciesCountries currency in currCountries)
                {
                    ...
                    if (selectedCurID != "0" && selectedCurID == currency.CurId && !hasItemSelected)
                    {
                        itm.Selected = true;
        ...
        private void GetSelectedDealData(int dealId)
        {
            ...
            if (deal != null)
            {
                ...
                else
                {
                    ...
                    TbxAmount.Text = GlobalMethods.FixNumberPattern(string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", deal.Amount));
                            ...
                            if (currency != null)
                            {
                                ...
                                if (vendorCurrencyID != "" && resellerCurrencyID != "")
                                {
                                    if (vendorCurrencyID != resellerCurrencyID)
                                    {
                                        ...
                                        {
                                            TbxAmount.Text = GlobalMethods.FixNumberPattern(string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", convertedAmount));
                                ...
                                if (currency != null)
                                {
                                    ...
                                    if (vendorCurrencyID != "" && resellerCurrencyID != "")
                                    {
                                        if (vendorCurrencyID != resellerCurrencyID)
                                        {
                                            ...
                                            if (convertedAmount > 0)
                                            {
                                                TbxAmount.Text = GlobalMethods.FixNumberPattern(string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", convertedAmount));       //convertedAmount.ToString();
                                            }
                                        }
                                        else
                                            TbxAmount.Text = GlobalMethods.FixNumberPattern(string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", deal.Amount));   //deal.Amount.ToString("0.00");
                                    }

                                    LoadCurrency(countryCurrency.CurId);
            ...
            try
            {
                if (vSession.User != null)
                {
                    ...
                    else if (vSession.User.CompanyType == EnumHelper.GetDescription(Types.Resellers).ToString())
                    {
                        ...
                        {
                            ShowPopUpModalWithText("Warning", "Please select Deal's result status", MessageTypes.Warning);
                            //GlobalMethods.ShowMessageControlDA(UcMessageControl, "Please select Deal's result status", MessageTypes.Warning, true, true, true, true, false);
                            //divOpportError.Visible = divOpportErrorBottom.Visible = true;
                    ...
                    if (mode == DealActionMode.UPDATE)
                    {
                        ...
                        if (deal != null)
                        {
                            if (vSession.User.CompanyType == EnumHelper.GetDescription(Types.Resellers).ToString())
                            {
                                ...
                                if (deal.DealResult != DealResultStatus.Pending.ToString())
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (subAccount != null)
                                        {
                                            EmailSenderLib.SendNewDealRegistrationWonLostEmail(subAccount.Email, vSession.User.CompanyName, DdlDealResult.Text, deal.CompanyName, true, vSession.Lang, session);
                                        }
                                        else
                                        {
                            ...
                            if (vSession.User.CompanyType == EnumHelper.GetDescription(Types.Resellers) && deal.ResellerId == vSession.User.Id)
                            {
                                ...
                                {
                                    if (deal.VendorId != 41078) // IS3 PARTNERS NOT SET TIER STATUS
                                    {
                        ...
                        if (deal != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (reseller != null)
                                {
                                    ...
                                    EmailSenderLib.SendNewDealRegistrationAcceptRejectEmail(reseller.Email, companyName, reseller.CompanyName, "Rejected", deal.CompanyName, false, vSession.Lang, session);
                                }
                                else
                        ...
                        if (deal != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (reseller != null)
                                {
                                    ...
                                    EmailSenderLib.SendNewDealRegistrationAcceptRejectEmail(reseller.Email, companyName, reseller.CompanyName, "Approved", deal.CompanyName, false, vSession.Lang, session);
                                }
                                else
                            ...
                            if (selectedCurrency != null)
                            {
                                ...
                                if (dealId > 0)
                                {
                                    ...
                                    if (deal != null && deal.CurId != "")
                                    {
                                        ...
                                        {
                                            if (vendorCurrencyID == resellerCurrencyID)
                                                TbxAmount.Text = GlobalMethods.FixNumberPattern(string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", deal.Amount));       //deal.Amount.ToString();
                                            else
                                            {
                                                double convertedAmount = ConverterLib.Convert(Convert.ToDouble(string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", TbxAmount.Text)), resellerCurrencyID, vendorCurrencyID);       //ConverterLib.Convert(Convert.ToDouble(TbxAmount.Text), resellerCurrencyID, vendorCurrencyID);
                                                if (convertedAmount > 0)
                                                {
                                                    TbxAmount.Text = GlobalMethods.FixNumberPattern(string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", convertedAmount));       //convertedAmount.ToString();
                                                }
