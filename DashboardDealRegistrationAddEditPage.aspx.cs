...
{
    public partial class DashboardDealRegistrationAddEditPage : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (vSession.User != null)
                {
                    if (vSession.User.CompanyType == Types.Vendors.ToString())
                    {
                        ...
                        {
                            Response.Redirect(ControlLoader.Dashboard(vSession.User, "deals-add-edit"), false);
                            return;
                        }
                        else if (vSession.User.Id == CustomVendorID)
                        {
                            Response.Redirect(ControlLoader.Dashboard(vSession.User, "deals-add-edit-365"), false);
                            return;
                        }
                    }
                    else
        ...
        private void GetCollaborationUsersTable()
        {
        ...
        private void GetSelectedDealData(int dealId)
        {
            ...
            if (deal != null)
            {
                ...
                if (!divCurrencyArea.Visible)
                    TbxAmount.Text = Thread.CurrentThread.CurrentCulture.NumberFormat.CurrencySymbol + " " + string.Format("{0:000" + Thread.CurrentThread.CurrentCulture.NumberFormat.NumberDecimalSeparator + "00}", deal.Amount);
                else
                {
            ...
            try
            {
                if (vSession.User != null)
                {
                    ...
                    if (vSession.User.CompanyType == Types.Vendors.ToString())
                    {
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please select if deal is Approved or Rejected", MessageTypes.Error);
                            //GlobalMethods.ShowMessageControlDA(UcMessageAlert, "Please select if deal is Approved or Rejected", MessageTypes.Error, true, true, true, true, false);
                        ...
                        else
                        {
                            ...
                            {
                                ShowPopUpModalWithText("Validation warning", "Please select Deal's duration", MessageTypes.Error);
                                //GlobalMethods.ShowMessageControlDA(UcMessageAlert, "Please select Deal's duration", MessageTypes.Error, true, true, true, true, false);
                    ...
                    else if (vSession.User.CompanyType == EnumHelper.GetDescription(Types.Resellers).ToString())
                    {
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please select Deal's status", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please select Deal's result status", MessageTypes.Error);
                            //GlobalMethods.ShowMessageControlDA(UcMessageAlert, "Please select Deal's result status", MessageTypes.Error, true, true, true, true, false);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Customer first name", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Customer last name", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Organisation name", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Organisation location", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please enter Email", MessageTypes.Error);
                        ...
                        else
                        {
                            ...
                            {
                                ShowPopUpModalWithText("Validation warning", "Please enter a valid Email Address", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Organisation website", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Organisation phone", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add deal's amount", MessageTypes.Error);
                        ...
                        else
                        {
                            try
                            {
                                ...
                                {
                                    ShowPopUpModalWithText("Validation warning", "Please add a valid deal's amount", MessageTypes.Error);
                                ...
                                {
                                    ShowPopUpModalWithText("Validation warning", "Please add positive deal's amount", MessageTypes.Error);
                        ...
                        if (divCurrencyArea.Visible)
                        {
                            ...
                            {
                                ShowPopUpModalWithText("Validation warning", "Please select your currency", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Organisation description", MessageTypes.Error);
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Please add Deal Forecasting", MessageTypes.Error);
                        ...
                        if (mode == DealActionMode.INSERT)
                        {
                            ...
                            {
                                ShowPopUpModalWithText("Validation warning", "Please add deal expected closed date", MessageTypes.Error);
                            ...
                            {
                                if (RdpExpectedClosedDate.SelectedDate.Value < DateTime.Now)
                                {
                                    ShowPopUpModalWithText("Validation warning", "Please add deal expected closed date after today", MessageTypes.Error);
                    ...
                    if (domain == "")
                    {
                        ShowPopUpModalWithText("Validation warning", "Sorry, something went wrong. Please try again later, or contact with us!", MessageTypes.Error);
                        //GlobalMethods.ShowMessageControlDA(UcMessageAlert, "Sorry, something went wrong. Please try again later, or contact with us!", MessageTypes.Error, true, true, true, true, false);
                    ...
                    if (mode == DealActionMode.UPDATE)
                    {
                        ...
                        {
                            ShowPopUpModalWithText("Validation warning", "Sorry, but this domain is already saved by you to other deal!", MessageTypes.Info);
                            ...
                            if (vSession.User.CompanyType == EnumHelper.GetDescription(Types.Resellers).ToString())
                            {
                                ...
                                if (deal.DealResult != DealResultStatus.Pending.ToString())
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (vendor != null)
                                            EmailSenderLib.SendNewDealRegistrationWonLostEmail(vendor.Email, vSession.User.CompanyName, DdlDealResult.SelectedItem.Text, deal.CompanyName, false, vSession.Lang, session);
                                        else
                    ...
                    else if (mode == DealActionMode.INSERT)
                    {
                        if (vendorId != -1 && vendorId != 0)
                        {
                            ...
                            if (collaborationId != -1)
                            {
                                ...
                                try
                                {
                                    if (vSession.User.CompanyType == EnumHelper.GetDescription(Types.Resellers))
                                    {
                                        ...
                                        if (subAccount != null)
                                        {
                                            EmailSenderLib.SendNewDealRegistrationEmail(subAccount.Email, vSession.User.CompanyName, true, vSession.Lang, session);
                                        }
                                        else
                                        {
                            ...
                            {
                                ShowPopUpModalWithText("Error", "Sorry, something went wrong. Please try again later, or contact with us!", MessageTypes.Error);
                                //GlobalMethods.ShowMessageControlDA(UcMessageAlert, "Sorry, something went wrong. Please try again later, or contact with us!", MessageTypes.Error, true, true, true, true, false);
                    ...
                    if (Request.QueryString["dealViewID"] != null)
                    {
                        ...
                        if (deal != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (reseller != null)
                                {
                                    string companyName = vSession.User.CompanyName;
                                    if (vSession.LoggedInSubAccountRoleID > 0 && vSession.SubAccountEmailLogin != "")
                                    {
                                        ElioUsersSubAccounts subAccount = Sql.GetSubAccountByEmail(vSession.SubAccountEmailLogin, session);
                                        if (subAccount != null && subAccount.AccountStatus == (int)AccountStatus.Completed)
                                        {
                                            if (subAccount.LastName != "" && subAccount.FirstName != "")
                                                companyName = subAccount.LastName + " " + subAccount.FirstName;
                                        }
                                    }

                                    EmailSenderLib.SendNewDealRegistrationAcceptRejectEmail(reseller.Email, companyName, reseller.CompanyName, "Rejected", deal.CompanyName, vSession.LoggedInSubAccountRoleID > 0, vSession.Lang, session);
                                }
                                else
                        ...
                        if (deal != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (reseller != null)
                                {
                                    ...
                                    EmailSenderLib.SendNewDealRegistrationAcceptRejectEmail(reseller.Email, companyName, reseller.CompanyName, "Approved", deal.CompanyName, vSession.LoggedInSubAccountRoleID > 0, vSession.Lang, session);
                                }
                                else
