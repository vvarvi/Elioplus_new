...
{
    public partial class FullRegistrationPartnerPrm : System.Web.UI.UserControl
    {
        ...
        private void UpdateStrings()
        {
            ...
            label1.Text = Localizer.GetTranslationResults(vSession.Lang, new XElementInfo("controls", "fullregistration", "label", "14")).Text;
            ...
            Label label4 = (Label)ControlFinder.FindControlRecursive(RpbRegistrationSteps, "Label4");
            label4.Text = Localizer.GetTranslationResults(vSession.Lang, new XElementInfo("controls", "fullregistration", "label", "15")).Text;
            ...
            lblGoPremium.Text = btnGoPremium.Text = "GO FREE";        //Localizer.GetTranslationResults(vSession.Lang, new XElementInfo("content", "pricing", "button", "6")).Text;
            ...
            if (vSession.User != null)
            {
                ...
                else
                {
                    try
                    {
                        if (vSession.User != null)
                        {
                            bool existCollaborationInvitation = vSession.User.UserApplicationType == (int)UserApplicationType.Collaboration;
                                ...
                                if (user != null)
                                {
                                    if (user.AccountStatus == (int)AccountStatus.NotCompleted)
                                    {
                                        try
                                        {
                                            ...
                                            //user.IsPublic = (int)AccountStatus.Public;
                                        ...
                                        //bool existCollaborationInvitationFromVendor = false;
                                        bool isInvitation = false;
                                        bool isRequest = false;

                                        if (user.CompanyType == EnumHelper.GetDescription(Types.Resellers).ToString())
                                        {
                                            ///* Comment out for Accepting Invitation

                                            //if (existCollaborationInvitation)
                                            //{
                                            ElioUsers vendor = Sql.GetUserById(VendorID, session);
                                            if (vendor != null)
                                            {
                                                ElioUserPacketStatus vendorPacketStatus = Sql.GetUserPacketStatusFeatures(vendor.Id, session);
                                                if (vendorPacketStatus != null)
                                                {
                                                    if (vendorPacketStatus.AvailableManagePartnersCount > 0)
                                                    {
                                                        #region Get Pending Invitation and Accept it if is invitation and no request

                                                        ElioCollaborationVendorsResellers collaboration = SqlCollaboration.GetInvitationByResellerByVendorByStatus(user.Id, vendor.Id, CollaborateInvitationStatus.Pending.ToString(), session);

                                                        if (collaboration != null)
                                                        {
                                                            //existCollaborationInvitationFromVendor = true;

                                                            ElioCollaborationVendorResellerInvitations invitation = SqlCollaboration.GetVendorResellerInvitationByResellerByVendResIdByStatusAndRecipientEmail(collaboration.Id, user.Email, CollaborateInvitationStatus.Pending.ToString(), session);
                                                            if (invitation != null)
                                                            {
                                                                ...
                                                                else if (invitation.UserId == vendor.Id)
                                                                {
                                                                    isInvitation = true;
                                                                    isRequest = false;
                                                                }
                                                                ...
                                                                {
                                                                    #region Update to Confirmed the Collaboration/Invitation

                                                                    #region Update Collaboration Status
                                                                    ...
                                                                    packetStatusLoader.Update(vendorPacketStatus);

                                                                    #endregion
                                                                }
                                                            }

                                                            if (invitation != null && invitation.InvitationStepDescription == CollaborateInvitationStatus.Confirmed.ToString())
                                                            {
                                                                #region Send Accept Notification Email

                                                                try
                                                                {
                                                                    EmailSenderLib.SendEmailToCollaboratePartnerForAcceptInvitation(vendor.Email, user.CompanyName, user.CompanyType, Request, false, vSession.Lang, session);
                                                                }
                                                                catch (Exception ex)
                                                                {
                                                                    Logger.DetailedError(Request.Url.ToString(), ex.Message.ToString(), ex.StackTrace.ToString());
                                                        ...
                                                        {
                                                            //no invitation found
                                                            Logger.Info("Channel Partner {0} with ID:{1}, completed successfully his registration from prm full registration page at {2}, but no collaboration invitation from his Vendor {3} with ID:{4} to confirmed was found.", vSession.User.CompanyName, vSession.User.Id, DateTime.Now.ToString(), vendor.CompanyName, vendor.Id);
                                                        }
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        //no available partner to manage count left
                                                        Logger.Info("Channel Partner {0} with ID:{1}, completed successfully his registration from prm full registration page at {2}, but his Vendor {3} with ID:{4} has no available partners to manage features left, so invitation could not be accepted", vSession.User.CompanyName, vSession.User.Id, DateTime.Now.ToString(), vendor.CompanyName, vendor.Id);
                                                    }
                                                }
                                                else
                                                {
                                                    //no packet status features found
                                                    Logger.Info("Channel Partner {0} with ID:{1}, completed successfully his registration from prm full registration page at {2}, but his Vendor {3} with ID:{4} has no packet status features.", vSession.User.CompanyName, vSession.User.Id, DateTime.Now.ToString(), vendor.CompanyName, vendor.Id);
                                                }
                                            }
                                            else
                                            {
                                                //no vendor found
                                                Logger.Info("Channel Partner {0} with ID:{1}, completed successfully his registration from prm full registration page at {2}, but no Vendor was found.", vSession.User.CompanyName, vSession.User.Id, DateTime.Now.ToString());
                                            }
                                            //}
                                            //*/
                                            ...
                                            if (user.CompanyType == EnumHelper.GetDescription(Types.Resellers).ToString())
                                            {
                                                ...
                                                //Response.Redirect(ControlLoader.Dashboard(vSession.User, "manage-partners"), false);

                                                #endregion
                                            }
                                            else
                                            {
                                                #region Redirect to Invitations Page
                                        ...
                                        try
                                        {
                                            ...
                                            if (vendor != null)
                                            {
                                                if (isRequest && !isInvitation)
                                                {
                                                    #region Send Request Email To Vendor

                                                    EmailSenderLib.CollaborationInvitationEmailFromChannelPartner((int)UserApplicationType.Collaboration, vendor.Email.Trim(), vendor.CompanyName, user.CompanyLogo, user.CompanyName, Request, "en", session);

                                                    #endregion
                                                }
                                                //else if (isInvitation && !isRequest)
                                                //{
                                                #region Send Invitation Email To Vendor

                                                //    EmailSenderLib.CollaborationInvitationEmailFromChannelPartner(user.UserApplicationType, vendor.Email.Trim(), vendor.CompanyName, user.CompanyName, "en", session);
                                                ...
                                                EmailSenderLib.SendNotificationEmailForNewVendorPartnerFullRegisteredUser(vSession.User, vendor.CompanyName, vSession.Lang, session);
