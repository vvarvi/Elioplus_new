...
{
    public partial class ViewClearbitLogFiles : System.Web.UI.UserControl
    {
        private ElioSession vSession = new ElioSession();
        DBSession session = new DBSession();

        private static string clearbitLogFilePath = System.Configuration.ConfigurationManager.AppSettings["LogPathClearBit"];
        ...
        protected void ViewLogs_OnItemDataBound(object sender, GridItemEventArgs e)
        {
            try
            {
                ...
                if (e.Item is GridDataItem)
                {
                    ...
                    aViewLogFiles.HRef = aViewLogFilesText.HRef = System.Configuration.ConfigurationManager.AppSettings["ViewClearbitLogFilesPath"] + "/" + item["fileName"].Text;
                    ...
                    url = url + HttpContext.Current.Request.Url.Authority + "/Logs/ClearBitLogs/" + item["fileName"].Text;

                    //hpLnkBtnPreViewLogFiles.NavigateUrl = System.Configuration.ConfigurationManager.AppSettings["ViewLogFilesPath"] + "/" + item["fileName"].Text;
        ...
        protected void ViewLogs_OnNeedDataSource(object sender, EventArgs args)
        {
            try
            {
                ...
                if (vSession.User != null)
                {
                    ...
                    DirectoryInfo logFilesDirectory = new DirectoryInfo(FileHelper.AddRootToPath(clearbitLogFilePath));
        ...
        protected void ImgBtnDeleteLogFiles_OnClick(object sender, EventArgs args)
        {
            try
            {
                ...
                if (vSession.User != null)
                {
                    ...
                    DirectoryInfo filesInDirectory = new DirectoryInfo(FileHelper.AddRootToPath(clearbitLogFilePath));
                    bool deleted = false;
