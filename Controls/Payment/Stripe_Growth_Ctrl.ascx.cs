...
{
    public partial class Stripe_Growth_Ctrl : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (vSession.User != null)
                {
                    if (!IsPostBack)
                    {
                        LblTotalCostValue.Text = Sql.GetPacketTotalCostWithVat(Convert.ToInt32(Packets.PremiumGrowth), session).ToString() + " $";
