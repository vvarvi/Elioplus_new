...
{
    public partial class DashboardDealRegistrationPage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (vSession.User != null)
                {
                    ...
                    bool hasRight = ManagePermissions.ManagePermissionsRights(vSession.User.Id, vSession.LoggedInSubAccountRoleID, vSession.IsAdminRole, "DashboardDealRegistrationPage", Actions.View, session);
                    if (!hasRight)
                    {
        ...
        private void GetCollaborationUsersDataSet()
        {
            DataSet usersDs = SqlCollaboration.GetCollaborationAssignAndFreeUsersByUserTypeDS(vSession.User, vSession.LoggedInSubAccountRoleID, vSession.SubAccountEmailLogin, vSession.IsAdminRole, CollaborateInvitationStatus.Confirmed.ToString(), session);

            if (usersDs.Tables.Count > 0)
            {
                if (usersDs.Tables[0].Rows.Count > 0 || usersDs.Tables[1].Rows.Count > 0)
                {
                    ...
                    if (usersDs.Tables[0].Rows.Count == 1 && usersDs.Tables[1].Rows.Count == 0)
                    {
                        DrpPartners.DataSource = usersDs.Tables[0];
                        ...
                        DrpPartners.Items.FindByValue(usersDs.Tables[0].Rows[0]["id"].ToString()).Selected = true;
                        DrpPartners.SelectedItem.Value = usersDs.Tables[0].Rows[0]["id"].ToString();
                        DrpPartners.SelectedItem.Text = usersDs.Tables[0].Rows[0]["company_name"].ToString();

                        DrpPartners.Enabled = false;
                    }
                    else if (usersDs.Tables[1].Rows.Count == 1 && usersDs.Tables[0].Rows.Count == 0)
                    {
                        DrpPartners.DataSource = usersDs.Tables[1];
                        ...
                        DrpPartners.Items.FindByValue(usersDs.Tables[1].Rows[0]["id"].ToString()).Selected = true;
                        DrpPartners.SelectedItem.Value = usersDs.Tables[1].Rows[0]["id"].ToString();
                        DrpPartners.SelectedItem.Text = usersDs.Tables[1].Rows[0]["company_name"].ToString();

                        DrpPartners.Enabled = false;
                    }
                    else if (usersDs.Tables[0].Rows.Count > 0 && usersDs.Tables[1].Rows.Count > 0)
                    {
                        DataRow row = usersDs.Tables[0].NewRow();
                        row["id"] = "0";
                        row["company_name"] = (vSession.User.CompanyType == Types.Vendors.ToString()) ? "Select Reseller" : "Select Vendor";
                        usersDs.Tables[0].Rows.Add(row);

                        EnumerableRowCollection<DataRow> assignedUsers = from user in usersDs.Tables[0].AsEnumerable()
                                                                         orderby user.Field<int>("id")
                                                                         select user;

                        DataTable tbl = new DataTable();
                        tbl.Columns.Add("id");
                        tbl.Columns.Add("company_name");

                        foreach (DataRow assignUsr in assignedUsers)
                        {
                            DataRow tblRow = tbl.NewRow();
                            tblRow["id"] = assignUsr["id"];
                            tblRow["company_name"] = assignUsr["company_name"];

                            tbl.Rows.Add(tblRow);
                        }

                        if (usersDs.Tables[1].Rows.Count > 0)
                        {
                            EnumerableRowCollection<DataRow> unassignedUsers = from user in usersDs.Tables[1].AsEnumerable()
                                                                               orderby user.Field<int>("id")
                                                                               select user;

                            foreach (DataRow unAssignUsr in unassignedUsers)
                            {
                                DataRow tblRow = tbl.NewRow();
                                tblRow["id"] = unAssignUsr["id"];
                                tblRow["company_name"] = unAssignUsr["company_name"];

                                tbl.Rows.Add(tblRow);
                            }
                        }

                        //DataView dv = assignedUsers.AsDataView();

                        DrpPartners.DataSource = tbl;
                        DrpPartners.DataValueField = "id";
                        DrpPartners.DataTextField = "company_name";
        ...
        private void GetCollaborationAllUsersTable()
        {
            DataTable usersTbl = SqlCollaboration.GetCollaborationAssignAndFreeUsersByUserTypeTbl(vSession.User, vSession.LoggedInSubAccountRoleID, vSession.SubAccountEmailLogin, vSession.IsAdminRole, CollaborateInvitationStatus.Confirmed.ToString(), session);
        ...
        private void SetDealLinkUrl(RepeaterItem item)
        {
            if (item != null)
            {
                ...
                aEdit.Visible = ManagePermissions.ManagePermissionsRights(vSession.User.Id, vSession.LoggedInSubAccountRoleID, vSession.IsAdminRole, "DashboardDealRegistrationPage", Actions.Edit, session);
                if (aEdit.Visible)
                {
                    string url = "";
                ...
                if (args.Item.ItemType == ListItemType.Item || args.Item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        if (vSession.User != null)
                        {
                            ...
                            aDelete.Visible = ManagePermissions.ManagePermissionsRights(vSession.User.Id, vSession.LoggedInSubAccountRoleID, vSession.IsAdminRole, "DashboardDealRegistrationPage", Actions.Delete, session);
                            if (aDelete.Visible)
                            {
